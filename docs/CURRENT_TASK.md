...
- Manual task entry is a standalone green 72px composer between the recording and task panels on mobile. It now shares TAP TO RECORD's 90s control language while retaining its green identity: a 3px solid ink frame, 5px hard ink shadow, framed yellow plus/mic/arrow tiles, bold title hierarchy, and a type-or-dictate subfield. It retains direct typing, a dedicated mic that dictates into the new-task field, and a real submit action.
...
- Browser QA passed for the standalone manual-task card: it renders between the recording and task panels, the task list remains in its own panel, and submitting the manual composer creates a new TO DO row.
...
- The Android handoff includes a safe-area-aware mobile bottom inset for the Spin the Wheel card. The manual composer is intentionally the one framed full-width control between the recording and task panels, using the same black frame/shadow language as TAP TO RECORD rather than a detached spacer band.
- Browser QA confirmed the workspace bands remain flush at `x: 0` and full viewport width at a 640px Android-style viewport; the manual composer intentionally retains its own 3px frame and 5px hard shadow, the body background is cream, and there is no horizontal overflow. A simulated Android speech bridge filled the manual task field from the dedicated mic control.
- Browser QA at a mobile Android-style viewport confirmed the exact current dimensions: static top banner `42px`, manual composer `72px` with 42px framed controls, transcript actions about `57px`, compact TO DO/DONE controls `29px`, centered Spin the Wheel launch band `76px`, and no horizontal overflow.
...
- Browser QA confirmed the standalone manual composer now shares TAP TO RECORD's framed control language while retaining its green body: a 3px solid ink frame, 5px hard ink shadow, framed yellow plus/mic/arrow controls, and a bold title/subline hierarchy. At 320px it remains exactly 72px high, full-width, and free of horizontal overflow; a real type-and-submit flow created a new TO DO row.
...
- APK SHA-256: `E713637115DCE63534875CA56793B3454EB63C1168D36B44CC8C3458C540E6CD` (1,479,886 bytes).
- Google Drive APK (new named file; prior versions preserved): https://drive.google.com/file/d/1Q1BUe-muPU9odWGyIbUpNk_0TH6xH24W/view?usp=drivesdk
